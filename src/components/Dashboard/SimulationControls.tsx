...
export default function SimulationControls({ map }: SimulationControlsProps) {
  const [emergencyLocation, setEmergencyLocation] = useState<Location | null>(null);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.code === 'Space') {
        e.preventDefault();
        if (cursorPosition) {
          setEmergencyLocation({ lat: cursorPosition.lat, lng: cursorPosition.lng });
        }
      }
    };
    ...
    <div className="absolute top-4 left-4 bg-white rounded-lg shadow-lg p-4 space-y-3">
      ...
      <div className="text-sm space-y-1">
        <p className="text-gray-600 text-xs">
          Press <kbd className="px-2 py-1 bg-gray-100 rounded border">Ctrl+Space</kbd> to place emergency
        </p>
        <p className={emergencyLocation ? 'text-green-600' : 'text-gray-400'}>
          {emergencyLocation ? '✓ Emergency location set' : '○ No emergency location'}
        </p>
      </div>
